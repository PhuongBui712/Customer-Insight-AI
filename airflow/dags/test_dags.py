...
with DAG(
    ...
    def print_project_root(project_root_path):
        ...
        # import
        from data_etl import (PROJECT_DIRECTORY,
                              analyse_customer_message_pipeline,
                              get_project_item_path, load_table)

        # run
        # analyse_customer_message_pipeline()
        df = load_table(
            get_project_item_path("data/message_table.csv"),
            list_cols=["user", "purpose"],
            datetime_cols=["inserted_at"],
        )
        print(df.shape)
