...
def load_worksheet(
    sheet_name: str, return_type: Literal["sheet", "dataframe"] = "dataframe"
) -> DataFrame:
    """
...
def update_worksheet(
    ...
    if mode == "replace":
        try:
            ...
            worksheet.update(
                [last_df.columns.values.tolist()] + last_df.values.tolist()
            )
    ...
    for col in upload_df.columns:
        if upload_df[col].apply(lambda x: isinstance(x, list)).any():
            upload_df[col] = upload_df[col].apply(
...
def quantify_data(
    message_df: DataFrame, user_col: str = "user", purpose_col: str = "purpose"
) -> DataFrame:
    ...
    count_purpose = count_purpose.value_counts(
        purpose_col, ascending=False
    ).reset_index()
