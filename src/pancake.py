...
def call_pancake_api(
    ...
    **kwargs: Any,
    ...
    request_args = {"url": url, "params": request_params}
...
def get_page(
    return_type: Literal["id", "standard", "full"] = "standard"
) -> Union[dict, list]:
    request_page_list_url = "https://pages.fm/api/v1/pages"
    response = call_pancake_api(url=request_page_list_url, return_type="dictionary")
    ...
    if return_type == "full":
        ...
        return {
            page["id"]: {
                "name": page["name"],
                "page_access_token": page["settings"].get("page_access_token", None),
            }
...
def get_page_access_token(page_info: dict) -> tuple[dict[str, str], list[str]]:
    ...
    request_page_access_token = lambda page_id: call_pancake_api(
    ...
    with ThreadPoolExecutor(max_workers=NUM_WORKERS) as executor:
        future_to_access_token = {
            executor.submit(request_page_access_token, id): id for id in page_ids
        }

        for future in tqdm(
            concurrent.futures.as_completed(future_to_access_token),
            total=len(future_to_access_token),
            desc="Generate page access tokens",
        ):
            ...
            if not response["success"]:
                print(
                    f'UserWarning: Cannot generate access token for page "{page_info[page_id]['name']}"'
                )
...
def get_page_conversations(
    ...
    for s, u in timestamp_list:
        ...
        while True:
            ...
            if not response["success"]:
                print(
                    f"UserWarning: Failed to get conversations for page {page_id}. Respone's message: {response['message']}"
                )
                break
...
def get_sender(
    sender_dict: dict, is_sender_patterns: Optional[list] = None
) -> Literal["customer", "admin"]:
